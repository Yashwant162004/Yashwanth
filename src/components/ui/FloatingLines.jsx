...
export default function FloatingLines({
  linesGradient = ['#34d3ee', '#6366f1', '#a855f7'],
  ...
  const getLineCount = (waveType) => {
    ...
    const idx = ['top', 'middle', 'bottom'].indexOf(waveType);
    return lineCount[idx] ?? 5;
  };

  const getLineDistance = (waveType) => {
    if (typeof lineDistance === 'number') return lineDistance;
    const idx = ['top', 'middle', 'bottom'].indexOf(waveType);
    return lineDistance[idx] ?? 5;
  };
  ...
  useEffect(() => {
    ...
    const utopCount = enabledWaves.includes('top') ? getLineCount('top') : 0;
    const umidCount = enabledWaves.includes('middle') ? getLineCount('middle') : 0;
    const ubotCount = enabledWaves.includes('bottom') ? getLineCount('bottom') : 0;

    const utopDist = enabledWaves.includes('top') ? getLineDistance('top') * 0.01 : 0.01;
    const umidDist = enabledWaves.includes('middle') ? getLineDistance('middle') * 0.01 : 0.01;
    const ubotDist = enabledWaves.includes('bottom') ? getLineDistance('bottom') * 0.01 : 0.01;
    ...
    const uniforms = {
      ...
      topLineCount: { value: utopCount },
      middleLineCount: { value: umidCount },
      bottomLineCount: { value: ubotCount },
      topLineDistance: { value: utopDist },
      middleLineDistance: { value: umidDist },
      bottomLineDistance: { value: ubotDist },
    ...
    if (interactive) {
      ...
      renderer.domElement.addEventListener('pointerleave', () => targetInfluenceRef.current = 0);
    }
  ...
  return <div ref={containerRef} className="absolute inset-0 pointer-events-none" style={{ mixBlendMode }} />;
}
